...
- `.github/workflows/robinhood-testnet-deploy.yml` is the secret-isolated operator lane for that sequence. It is manual, main-only, bound to the `robinhood-testnet` GitHub environment, requires the exact generic-core confirmation phrase, and uploads disabled operator evidence without exposing the deployer key. It never targets Robinhood mainnet.
- `robinhood:generic:testnet:release:prepare` binds the exact clean commit, deployed test asset, passing two-provider qualification record, explicit testnet approval record, and generated contract hashes into the ignored release manifest. It writes exclusively and refuses records that enable production.
