...
- Robinhood artifact generation requires every `contracts/robinhood/**/*.sol` file and `hardhat.config.ts` to be committed. The recorded `sourceCommit` is the newest ancestor that contains the exact current source tree, so a later artifact-only commit does not create a self-referential `HEAD` provenance claim.
